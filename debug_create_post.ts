...
async function main() {
    ...
    // 1. Get User Sarah
    const user = await prisma.user.findUnique({ where: { email: 'sarah@example.com' } });
    ...
    const postData = {
        ...
        title: 'Debug API Post',
        notes: 'Testing creation via script',
        status: 'PLANNED'
    };
    ...
    if (res.status === 403) {
        console.error('❌ 403 Forbidden! API logic failed explicitly.');
    } else if (res.ok) {
        console.log('✅ Post Created!');
        // cleanup
        if (data.data?.id) {
            await prisma.post.delete({ where: { id: data.data.id } });
